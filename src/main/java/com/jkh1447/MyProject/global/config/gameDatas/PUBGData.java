  ...
  public GameInfo getGameInfo() {
    ...
    List<GameConditionDto> pubgConditions = List.of(
      ...
      GameConditionDto.builder().id("viewMode").label("시점").type("select")
        .options(List.of("FPP", "TPP")).defaultValue("TPP").
        dependsOn("mode").dependsOnValues(List.of("랭크", "일반전")).build()
    );
    

    return GameInfo.builder().id(PUBGConstants.GAMENAME).name("PUBG")
        .description("배틀로얄 슈팅 게임").playerCount("4명")
        .conditions(pubgConditions)
        .build();
  }
}
