...
public class ChatingService {
    ...
    public ChatMessageDto createChatMessageDto(Long id, String roomId, ChatMessageDto chatMessage) {

        String roomKey = MatchingConstants.ROOM_PARTICIPANTS_KEY + roomId;
        ...
        switch (chatMessage.type()) {
            case ENTER:
                message = ChatMessageDto.createEnterMessage(id, roomId, senderId, safeNickname);
                break;
            case TALK:
                ...
                if (rawContent.length() > ChatingConstants.MESSAGE_LENGTH_LIMIT) {
                    throw new IllegalArgumentException(
                            ChatingConstants.MESSAGE_LENGTH_LIMIT_MESSAGE);
                }

                String safeContent = HtmlUtils.htmlEscape(rawContent);
                message = ChatMessageDto.createTalkMessage(id, roomId, senderId, safeNickname,
                        safeContent);
                break;
            case LEAVE:
                message = ChatMessageDto.createLeaveMessage(id, roomId, senderId, safeNickname);
    ...
    public Long saveChatMessage(ChatMessageDto message, String clientIp, String userAgent) {
        ChatMessage chatMessage = ChatMessage.builder().roomId(message.roomId())
                ...
                .content(message.content()).clientIp(clientIp).userAgent(userAgent).build();
